...
const tempDir = path.resolve(__dirname, "../src");

const managerRender = manager => {
    let temp = fs.readFileSync(path.resolve(tempDir, "manager.html"), "utf8");
...
const engineerRender = engineer => {
    let temp = fs.readFileSync(path.resolve(tempDir, "engineer.html"), "utf8");
...
const internRender = intern => {
    let temp = fs.readFileSync(path.resolve(tempDir, "intern.html"), "utf8");
...
const mainRender = html => {
    const temp = fs.readFileSync(path.resolve(tempDir, "index.html"), "utf8");
